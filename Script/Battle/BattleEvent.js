class BattleEvent {
  ...
  actionMenu(resolve) {
    const menu = new ActionMenu({
      ...
      replacement: Object.values(this.battle.combatants).filter((combatant) => {
        return (
          combatant.id !== this.event.caster.id &&
          combatant.team === this.event.caster.team
        );
      }),
      onComplete: (action) => {
  ...
  getAmountOfDamage(caster, target) {
    ...
    if (caster.buff?.buffAtk)
      amountOfDamage = amountOfDamage * caster.buff.buffAtk;
    if (caster.debuff?.debuffAtk)
      amountOfDamage = amountOfDamage * caster.debuff.debuffAtk;
  ...
  getAmountOfSpecialDamage(caster, target) {
    ...
    if (caster.buff?.buffAtkSpe)
      amountOfDamage = amountOfDamage * caster.buff.buffAtkSpe;
    if (caster.debuff?.debuffAtkSpe)
      amountOfDamage = amountOfDamage * caster.debuff.debuffAtkSpe;
  ...
  async stateChange(resolve) {
    const { caster, target, damage, specialDamage, recovery, power, status } =
      this.event;
  ...
  replacementMenu(resolve) {
    const allReplacer = Object.values(this.battle.combatants).filter(
      (combatant) => {
