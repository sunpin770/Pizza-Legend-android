...
class Overlord {
    ...
    checkHeroPosition() {
        document.addEventListener("IFinishWalk", (e)=>{
            if (e.detail.whoId === "hero") this.map.checkCaseEvent();
        });
    }
    startMap(mapSrc, heroInitialState = null, overworld) {
        let ancientMap;
        if (this.map) ancientMap = this.map;
    ...
    bindActionInput() {
        this.interactInput = new KeyPressListener("f", ()=>this.map.checkInteraction());
        this.pauseInput = new KeyPressListener("&", ()=>{
            if (!this.map.cutscenePlaying) this.map.startCutscene([
                {
                    type: "pause"
                }
            ]);
...
//# sourceMappingURL=index.44283561.js.map
