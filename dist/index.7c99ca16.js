class BattleEvent {
    ...
    textMessage(resolve) {
        const text = this.event.text.replace("{CASTER}", this.event.caster?.name).replace("{TARGET}", this.event.target?.name).replace("{ACTION}", this.event.attack?.name);
    ...
    async stateChange(resolve) {
        ...
        if (recovery) {
            if (typeof recovery === "number") this.amountOfHeal = recovery;
            else this.amountOfHeal = this.getAmountOfHeal(caster, target);
            let newHp = caster.hp + this.amountOfHeal;
            if (newHp > caster.maxHp) newHp = caster.maxHp;
            caster.update({
                hp: newHp
            });
        }
        if (status) {
            ...
            let whoIsBuff = who;
            if (statusConfig.buff) whoIsBuff = await this.teamMenu();
            ...
            if (statusConfig.buff || statusConfig.debuff) whoIsBuff.getBuff();
        }
        if (status === null) who.update({
            buff: null,
            debuff: null,
            specialDamage: null
        });
        ...
        let feedBackText;
        if (damage) feedBackText = this.getFeedBackMessage(efficiency);
    ...
    getFeedBackMessage(efficiency) {
        if (efficiency > 1) return "C'est super efficace";
        else if (efficiency < 1) return "C'est pas tr\xe8s \xe9fficace";
        else return null;
    }
}

//# sourceMappingURL=index.7c99ca16.js.map
