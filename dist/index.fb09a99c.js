class PlayerState {
    ...
    addPizza(pizzaId) {
        ...
        if (this.lineup.length < 4) this.lineup.push(newId);
        utils.createCustomEvent("lineupChanged");
    }
    statPizzaUpdate() {
        document.addEventListener("pizzaLvlUp", (e)=>{
            ...
            //Set up New Attack
            if (pizza.level > 6 && pizza.level % 5 === 0) getNewAttack();
            else if (pizza.level > 3 && pizza.level % 3 === 0) getNewAttack();
            else if (pizza.level < 3 && pizza.level % 2 === 0) getNewAttack();
        });
    }
    pizzaGetNewAttack(pizza) {
        let newAction;
        for(let i = 0; i < pizza.canGetActions.length; i++)//Check new Action who the pizza don't have, a optmiser en cherchant dans la boucle a chaque fois
...
//# sourceMappingURL=index.fb09a99c.js.map
