class ActionMenu {
    constructor({ caster , ennemy , items , replacement , onComplete  }){
        ...
        items.forEach((item)=>{
            if (item.team === this.caster.team) {
                if (!quantityItems[item.actionsId]) quantityItems[item.actionsId] = {
                    actionsId: item.actionsId,
                    quantity: 1,
                    instanceId: item.instanceId
                };
                else quantityItems[item.actionsId].quantity += 1;
            }
        });
    ...
    decide() {
        ...
        for(let i = 0; i < this.caster.actions.length; i++){
            ...
            if (efficiency > attackOptions.efficiency && !attack.buff && !attack.debuff && attack.specialDamage && attack.recovery) asignAttackOptions();
            else if (efficiency === attackOptions.efficiency && power > attackOptions.power && !attack.buff && !attack.debuff && attack.specialDamage && attack.recovery) asignAttackOptions();
        }
        this.onComplete({
    ...
    getPages() {
        const backOptions = {
            label: "Back",
            descriptions: "Revenir en arri\xe8re",
            handler: ()=>{
...
//# sourceMappingURL=index.0b45208d.js.map
